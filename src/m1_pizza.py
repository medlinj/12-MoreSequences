...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  3 purple dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 3, 'purple')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    x = generate_points_on_circle(circle,number_of_points)
    for k in range(number_of_points):
        point = x[k]
        small = rg.Circle(point, 10)
        small.fill_color = color
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  10 slices, thin (thickness=5) green lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 10, 'green', 5)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        outside_point = x[k]
        inside_point = circle.center
        line = rg.Line(outside_point, inside_point)
        line.color = color
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3: 12 segments with thick red lines.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 75)
    circle.outline_thickness = 4
    polygon(window, circle, 12, 'red', 4)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    y = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments - 1):
        ...
        line.attach_to(window)
    line2 = rg.Line(y[number_of_segments - 1], y[0])
    line2.color = color
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'FANCY POLYGON tests 4:  10 yellow lines, hops = 2.'
    window = rg.RoseWindow(520, 400, title)
    circle = rg.Circle(rg.Point(100, 100), 80)
    fancy_polygon(window, circle, 10, 2, 'yellow', 3)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    y = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        x = y[k]
        c = y[(k + hops_to_next_point) % number_of_lines]
        line = rg.Line(x, c)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
